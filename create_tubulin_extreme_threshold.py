#!/usr/bin/env python3
"""
Enhanced EMBL OME-Zarr Tubulin Visualization with Extreme Threshold (0.025)
This script creates the most sensitive 3D visualization of Tubulin fluorescence data
with threshold 0.025 to reveal absolute maximum microtubule detail.
"""
...
def create_interactive_3d_scatter_extreme_threshold(data, threshold=0.025, max_points=25000):
    """Create extreme sensitivity 3D scatter plot with threshold 0.025"""
    print(f"🎨 Creating extreme sensitivity 3D visualization (threshold={threshold})...")
    ...
    fig = go.Figure(data=go.Scatter3d(
        ...
        mode='markers',
        marker=dict(
            size=1.5,  # Extremely small markers for maximum detail
            color=colors,
            colorscale='Viridis',
            opacity=0.5,  # Low opacity for dense structure visibility
    ...
    fig.update_layout(
        title={
            'text': "Interactive 3D View: EMBL bmcc122 (Tubulin_Ch1) - Extreme Sensitivity<br>" +
                   "<sub>Threshold: 0.025 (Absolute maximum microtubule detail)</sub>",
            'x': 0.5,
            'font': {'size': 16}
        },
        scene=dict(
        ...
        annotations=[
            dict(
                text=f"Extreme sensitivity view with {len(z_coords)} data points<br>" +
                     "Threshold 0.025 reveals absolute finest microtubule network",
                x=0.02, y=0.98,
...
def main():
    ...
    print("🧬 EMBL OME-Zarr Extreme Sensitivity Tubulin Visualization")
    print("=" * 65)
    ...
    try:
        ...
        # Create extreme sensitivity 3D visualization
        fig = create_interactive_3d_scatter_extreme_threshold(tubulin_data, threshold=0.025, max_points=25000)
        
        # Create output directory
        output_dir = "embl_visualizations"
        os.makedirs(output_dir, exist_ok=True)
        
        # Save extreme sensitivity visualization
        output_file = os.path.join(output_dir, "interactive_3d_Tubulin_Ch1_extreme_threshold.html")
        fig.write_html(output_file)
        
        print(f"\n✅ Extreme sensitivity Tubulin visualization complete!")
        print(f"📁 File created: {output_file}")
        print(f"\n🔍 This extreme sensitivity visualization shows:")
        print(f"   • Absolute maximum microtubule detail with threshold 0.025")
        print(f"   • Complete cytoskeletal architecture")
        print(f"   • Weakest possible fluorescence signals")
        print(f"   • Full microtubule network at highest resolution")
